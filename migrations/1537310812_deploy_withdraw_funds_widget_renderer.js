...
module.exports = function(deployer) {
    return deployer.then(async () => {
        let renderer = await deployer.deploy(WithdrawFundsWidgetRenderer);
        await Promise.all([
            ...
            await renderer.setResource("en", "withdraw_funds", "자금 출금"),
            await renderer.setResource("en", "short_desc", "성공한 세일로부터 모금된 자금을 출금할 수 있습니다."),
            await renderer.setResource("en", "long_desc", "세일이 성공했기 때문에 자금을 출금할 수 있습니다. 모든 자금이 내 계좌로 전송됩니다."),
            await renderer.setResource("en", "withdraw", "출금하기"),
            await renderer.setResource("en", "withdraw_confirm", "출금하시겠습니까?"),
        ]);
    });
};
